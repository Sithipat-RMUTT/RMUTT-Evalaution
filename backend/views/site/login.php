...
(function () {
    ...
    if (tabStaff && tabAdmin) {
        tabStaff.addEventListener('click', function () {
            // If on backend, redirect to frontend login with portal=staff
            window.location.href = 'http://' + window.location.hostname + ':8000/index.php?r=site/login&portal=staff';
        });
    ...
    if (loginForm.length && submitBtn.length) {
        ...
        loginForm.on('afterValidate', function (event, messages, errorAttributes) {
            if (errorAttributes.length > 0) {
                ...
                var currentPortal = portalInput ? portalInput.value : 'admin';
